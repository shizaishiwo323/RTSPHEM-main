...
for iStep = 1:numel(stepResults)
    ...
    for iComponent = 1:numComponents
        rows(end + 1, :) = {iStep, accepted, dtSeconds, ...
            ...
            chemistryDelta(iComponent), residual(iComponent)}; %#ok<AGROW>
    end
end
if isempty(rows)
    tableOut = table();
...
for iStep = 1:numel(stepResults)
    ...
    for iComponent = 1:numComponents
        rows(end + 1, :) = {iStep, accepted, dtSeconds, ...
            ...
            roundoffSuppressed(iComponent), residual(iComponent)}; %#ok<AGROW>
    end
end
if isempty(rows)
    tableOut = table();
...
tableOut = table('Size', [0, numel(chemistryVariableNames())], ...
    ...
    'string', 'string', 'double', 'string', 'string', 'logical', 'string'}, ...
...
names = {'step_index', 'accepted', 'solid_volume_before_cm3', ...
    ...
    'realized_mineral_moles', 'max_displacement_over_h'};
end
...
names = {'step_index', 'accepted', 'converged', 'failed_cell_count', ...
    ...
    'phreeqc_run_method', 'phreeqc_session_reused', 'transaction_status'};
end
