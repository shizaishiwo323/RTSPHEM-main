...
validPairs = (referenceTable.source == "Zhang2010" & referenceTable.("case") == "experiment_25mM") | ...
    (referenceTable.source == "Yoon2012" & ismember(referenceTable.("case"), ["case_1", "case_5"]));
if any(~validPairs)
    error('RTSPHEM:Precipitate:InvalidReferenceCurves', ...
        'Reference curves contain invalid source/case pairings.');
end
validRegions = (referenceTable.source == "Zhang2010" & ...
    ismember(referenceTable.region, ["zhang_upgradient", "zhang_middle", "zhang_downgradient"])) | ...
    (referenceTable.source == "Yoon2012" & ...
    ismember(referenceTable.region, ["entire_domain", "first_pore", "first_three_pores"]));
if any(~validRegions)
    error('RTSPHEM:Precipitate:InvalidReferenceCurves', ...
        'Reference curves contain invalid source/case/region combinations.');
end
if any(~isfinite(referenceTable.time_min)) || ...
        any(~isfinite(referenceTable.precipitated_area_norm) & ~isfinite(referenceTable.precipitated_area_cm2))
    error('RTSPHEM:Precipitate:InvalidReferenceCurves', ...
...
groups = unique(referenceTable(:, {'source', 'case', 'region'}), 'rows', 'stable');
    ...
    if any(missingNorm)
        ...
        if ~isfinite(scale) || scale <= 0
            error('RTSPHEM:Precipitate:InvalidReferenceCurves', ...
                ...
                groups.source(iGroup), groups.("case")(iGroup), groups.region(iGroup));
        end
