...
if isempty(benchmarkComparisonTimes_s)
    benchmarkComparisonTimes_s = [];
else
    benchmarkComparisonTimes_s = sort(double(benchmarkComparisonTimes_s(:)));
end
benchmarkComparisonTimesSource = string(cfgget(config, ...
    'benchmarkComparisonTimesSource', 'unspecified'));
geometryFraming = string(cfgget(config, 'geometryFraming', ''));
if strlength(geometryFraming) == 0 && contains(lower(string(layoutType)), "zhang2010")
    geometryFraming = "approximate_local_cylindrical_post_layout_not_digitized_geometry";
end
...
if ~useExternalGeometry
    ...
    switch layoutType
        ...
        case {'zhang2010_micromodel_local', 'zhang2010_micromodel_full'}
            % Zhang/Yoon micromodel benchmark dimensions are supplied
            % directly by precip_ConfigureZhangYoonBenchmark. The first
            % implementation below uses a deterministic cylindrical-post
            % approximation; strict image-level geometry should later use
            % external DXF/TIF import from digitized Zhang/Yoon figures.
            lengthYAxis = targetLengthYAxis;
            lengthXAxis = targetLengthYAxis * targetAspectRatio;
...
flowDirection = lower(strrep(strtrim(char(cfgget(config, ...
    'flowDirection', 'left_to_right'))), '-', '_'));
flowBoundaryMode = lower(strrep(strtrim(char(cfgget(config, ...
    'flowBoundaryMode', 'uniform_left_inlet'))), '-', '_'));
if strcmpi(flowBoundaryMode, 'split_left_inlet') && ~strcmpi(flowDirection, 'left_to_right')
    error('RTSPHEM:Precipitate:InvalidSplitInletFlowDirection', ...
...
mineralEvolutionMode = lower(strrep(strtrim(char(cfgget(config, ...
    'mineralEvolutionMode', 'dissolution_positive_surface'))), '-', '_'));
useSignedCalciteSurface = usePhreeqc && any(strcmp(mineralEvolutionMode, ...
    {'signed_calcite_surface', 'precipitation_signed_calcite_surface'}));
...
[phreeqcTransportUpperBounds, phreeqcTransportSpeciesNames] = ...
    precip_ComputePhreeqcTransportUpperBounds(config, phreeqcTransportMaxFactor);
...
fprintf('Mineral evolution mode         : %s\n', mineralEvolutionMode);
fprintf('Flow boundary mode             : %s\n', flowBoundaryMode);
if usePhreeqc
...
if useExternalGeometry
    ...
    switch layoutType
        ...
        case {'zhang2010_micromodel_local', 'zhang2010_micromodel_full'}
            ...
            xStart = inletClearance + circleRadius;
            yStart = 0;
            xCentersBase = xStart : pitchX : maxX;
            yCenters = yStart : pitchY : maxY;
            circleCenters = [];
            for iRow = 1:numel(yCenters)
                xOffset = 0.5 * pitchX * mod(iRow - 1, 2);
                xRow = xCentersBase + xOffset;
                xRow = xRow(xRow <= maxX);
                yRow = yCenters(iRow) * ones(size(xRow));
                circleCenters = [circleCenters; [xRow(:), yRow(:)]]; %#ok<AGROW>
            end
            circleRadii = repmat(circleRadius, size(circleCenters, 1), 1);
            fprintf('=== Zhang/Yoon approximate cylindrical-post geometry ===\n');
            fprintf('  layoutType       : %s\n', layoutType);
            fprintf('  post diameter    : %.6f cm\n', postDiameter);
            fprintf('  pitchX / pitchY  : %.6f / %.6f cm\n', pitchX, pitchY);
            fprintf('  generated posts  : %d\n', size(circleCenters, 1));
            fprintf('  note             : approximate array; use digitized DXF/TIF for strict reproduction.\n');

        otherwise
            error('Unknown layoutType. Use square | hex | random | single_circle | zhang2010_micromodel_local | zhang2010_micromodel_full.');
    end
...
if strcmpi(flowBoundaryMode, 'split_left_inlet')
    validateSplitInletRangesOnLeftBoundary(gridHyPHM, config, EPS);
end
...
benchmarkAreaConfig = configureBenchmarkAreaWindows(config, layoutType);
initialBenchmarkSolidAreas = computeBenchmarkSolidAreaWindows( ...
    gridHyPHM, levelSet{1}(:, 1), benchmarkAreaConfig);
...
allowAdaptiveTimeGridExtensionDefault = targetSliceSettings.enabled && ~hasExplicitMaxTotalTimeSteps;
allowAdaptiveTimeGridExtension = precip_ResolveAdaptiveTimeGridExtension( ...
    config, allowAdaptiveTimeGridExtensionDefault);
...
if usePhreeqc
    ...
    calciumTransport = createPhreeqcTransport(gridHyPHM, transportStepper, flow, ...
        ...
        diffusionCoefficient, inletVelocity, EPS, 'exp', inletFluxCalcium);
    carbonTransport = createPhreeqcTransport(gridHyPHM, transportStepper, flow, ...
        ...
        diffusionCoefficient, inletVelocity, EPS, 'exp', inletFluxCarbon);
    sodiumTransport = createPhreeqcTransport(gridHyPHM, transportStepper, flow, ...
        ...
        diffusionCoefficient, inletVelocity, EPS, 'exp', inletFluxSodium);
    chlorideTransport = createPhreeqcTransport(gridHyPHM, transportStepper, flow, ...
        ...
        diffusionCoefficient, inletVelocity, EPS, 'exp', inletFluxChloride);
end
...
benchmarkComparisonCsvFile = fullfile(resultsDir, 'benchmark_comparison_times_log.csv');
precipAreaTimeseriesCsvFile = fullfile(resultsDir, 'precipitation_area_timeseries.csv');
precipAreaTimeseriesPngFile = fullfile(resultsDir, 'precipitation_area_timeseries.png');
benchmarkSnapshotDir = fullfile(resultsDir, 'benchmark_snapshots');
...
precip_InitializePrecipitationAreaTimeseries(precipAreaTimeseriesCsvFile);
...
if ~exist(benchmarkSnapshotDir, 'dir'); mkdir(benchmarkSnapshotDir); end
phreeqcOutputDir = '';
phreeqcWorkDir = '';
    ...
    if strcmpi(phreeqcRateLaw, 'tst_match')
        ...
        if useSignedCalciteSurface
            phreeqcCalciteRateData = precip_ComputeSignedCalciteInterfaceRatePerArea( ...
                phreeqcSpeciesData, initialPhreeqcGeometryState.interface_area_cm2, 1);
        else
            phreeqcCalciteRateData = ComputePhreeqcInterfaceRatePerArea( ...
                phreeqcSpeciesData, initialPhreeqcGeometryState.interface_area_cm2, ...
                1, phreeqcOptions, phreeqcSpeciesData.h_mol_cm3);
        end
    end
...
initializeBenchmarkComparisonLog(benchmarkComparisonCsvFile, benchmarkComparisonTimes_s);
benchmarkComparisonExported = false(size(benchmarkComparisonTimes_s));
benchmarkSnapshotExported = false(size(benchmarkComparisonTimes_s));
...
metadata.parameters = struct( ...
    ...
    'flowBoundaryMode', string(flowBoundaryMode), ...
    'mineralEvolutionMode', string(mineralEvolutionMode), ...
    'benchmarkComparisonTimes_s', benchmarkComparisonTimes_s, ...
    'benchmarkComparisonTimesSource', benchmarkComparisonTimesSource, ...
    'geometryFraming', geometryFraming, ...
...
metadata.outputs = struct( ...
    ...
    'benchmark_comparison_times_csv', string(benchmarkComparisonCsvFile), ...
    'benchmark_snapshots_dir', string(benchmarkSnapshotDir), ...
    'precipitation_area_timeseries_csv', string(precipAreaTimeseriesCsvFile), ...
    'precipitation_area_timeseries_png', string(precipAreaTimeseriesPngFile), ...
...
while transportStepper.next
    ...
    if usePhreeqc
        if strcmpi(phreeqcRateLaw, 'tst_match') && ~useSignedCalciteSurface
            phreeqcRateV = averagePositiveTriangleDataToVertices( ...
                phreeqcCalciteRateData, VertexTriMatrix, gridHyPHM.numV);
    ...
    if usePhreeqc
        ...
        phreeqcState = collectPhreeqcState(hydrogenTransport, calciumTransport, carbonTransport, ...
            sodiumTransport, chlorideTransport, timeIterationStep, phreeqcGeometryState);
        if strcmpi(phreeqcRateLaw, 'tst_match')
            positiveWaterVolumes = phreeqcGeometryState.water_volume_cm3( ...
                phreeqcGeometryState.water_volume_cm3 > 0);
            ...
            phreeqcState.reaction_water_volume_cm3 = max( ...
                phreeqcGeometryState.water_volume_cm3(:), reactionWaterFloorCm3);
            if useSignedCalciteSurface
                phreeqcCalciteRateData = precip_ComputeSignedCalciteInterfaceRatePerArea( ...
                    phreeqcSpeciesData, phreeqcGeometryState.interface_area_cm2, ...
                    macroscaleTimeStepSize);
            else
                phreeqcCalciteRateData = ComputePhreeqcInterfaceRatePerArea( ...
                    phreeqcSpeciesData, phreeqcGeometryState.interface_area_cm2, ...
                    macroscaleTimeStepSize, phreeqcOptions, phreeqcState.h_mol_cm3);
            end
            phreeqcState.prescribed_calcite_dissolved_moles = min( ...
                phreeqcCalciteRateData(:) ...
                .* phreeqcGeometryState.interface_area_cm2(:) ...
                .* max(macroscaleTimeStepSize, 0), ...
                max(phreeqcGeometryState.calcite_moles(:), 0));
        end
        try
            if useSignedCalciteSurface
                phreeqcSpeciesData = precip_RunPhreeqcCalciteBatchSigned( ...
            ...
            metadata.failure = struct( ...
                'terminationReason', "phreeqc_error", ...
                'timeStep', timeIterationStep, ...
                'time_s', currentTime, ...
                'targetEndTime_s', endTime, ...
                'adaptiveTimeGridAppendCount', adaptiveTimeGridAppendCount, ...
                'finalNumTimeSteps', transportStepper.numsteps, ...
                'finalTimeGrid_s', summarizeTimeGridForMetadata(timeSteps), ...
    ...
    diagnosticInletConcentrationScale = precip_ComputeBoundaryInletConcentrationScale( ...
        flowBoundaryMode, config, 'H_total', initialHydrogenConcentration);
    diagnosticInletMassFlux = precip_ComputeBoundaryInletMassFlux( ...
        flowBoundaryMode, config, 'H_total', initialHydrogenConcentration, ...
        inletVelocity, thickness, flowConfig.crossSectionLength);
    ...
    stabilityDiagnostics = computeStabilityDiagnostics( ...
        ...
        SorceScale, rateCoefficientTST, thickness, diagnosticInletConcentrationScale, ...
        diagnosticInletMassFlux, flowConfig, normalSpeedMax, por_now, perm_now, ...
    ...
    try
        precipAreaMetrics = precip_ComputePrecipitationAreaMetrics( ...
            gridHyPHM, levelSet{1}(:, timeIterationStep + 1), ...
            levelSet{1}(:, 1), config);
        precip_AppendPrecipitationAreaTimeseries( ...
            precipAreaTimeseriesCsvFile, timeIterationStep, t_now, precipAreaMetrics);
    catch ME
        warning('RTSPHEM:Precipitate:AreaTimeseriesWriteFailed', ...
            'Failed to append precipitation area time-series at step %d: %s', ...
            timeIterationStep, ME.message);
    end
    ...
    benchmarkComparisonExported = appendBenchmarkComparisonLogIfDue( ...
        benchmarkComparisonCsvFile, benchmarkComparisonTimes_s, benchmarkComparisonExported, ...
        timeIterationStep, t_now, por_now, perm_now, k_k0_now, rate_now, sa_now, vol_now, ...
        gridHyPHM, levelSet{1}(:, timeIterationStep + 1), benchmarkAreaConfig, ...
        initialBenchmarkSolidAreas);
    benchmarkSnapshotExported = precip_ExportBenchmarkSnapshots( ...
        benchmarkSnapshotDir, benchmarkComparisonTimes_s, benchmarkSnapshotExported, ...
        timeIterationStep, t_now, gridHyPHM, levelSet{1}(:, timeIterationStep + 1), ...
        levelSet{1}(:, 1), config);
    ...
    if usePhreeqc && (mod(timeIterationStep, phreeqcExportEvery) == 0 || doExportStep)
        precip_WritePhreeqcSpeciesTable(phreeqcOutputDir, timeIterationStep, t_now, gridHyPHM, phreeqcSpeciesData);
        if saveIndividualPlots
            precip_ExportPhreeqcSpeciesPlots(phreeqcSpeciesPlotDir, timeIterationStep, t_now, ...
    ...
    if ~stopAfterThisStep
        ...
        if enableConcentrationCflLimit && isfield(stabilityDiagnostics, 'cMax')
            [nextMacroscaleStepSize, wasConcentrationLimited, overshootRatio, activeConcentrationThreshold] = ...
                ...
                diagnosticInletConcentrationScale, concentrationOvershootThreshold, ...
        ...
        if shouldApplyNextStep
            ...
            elseif allowAdaptiveTimeGridExtension && remainingTime > EPS
                ...
                transportProblemsToExtend = {hydrogenTransport};
                if usePhreeqc
                    transportProblemsToExtend = {hydrogenTransport; calciumTransport; ...
                        carbonTransport; sodiumTransport; chlorideTransport};
                end
                precip_ExtendTransportProblemsToStepper(transportProblemsToExtend);
                adaptiveTimeGridAppendCount = adaptiveTimeGridAppendCount + 1;
...
try
    precip_PlotPrecipitationAreaTimeseries( ...
        precipAreaTimeseriesCsvFile, precipAreaTimeseriesPngFile, benchmarkComparisonTimes_s);
catch ME
    warning('RTSPHEM:Precipitate:AreaTimeseriesPlotFailed', ...
        'Failed to plot precipitation area time-series: %s', ME.message);
end
...
benchmarkSnapshotManifest = buildBenchmarkSnapshotManifest( ...
    benchmarkSnapshotDir, benchmarkComparisonTimes_s, benchmarkSnapshotExported, ...
    validTimeSteps(end), benchmarkComparisonCsvFile);
if benchmarkSnapshotManifest.numMissingDue > 0
    error('RTSPHEM:Precipitate:MissingBenchmarkSnapshots', ...
        'Missing %d due benchmark snapshot files in %s.', ...
        benchmarkSnapshotManifest.numMissingDue, benchmarkSnapshotDir);
end
...
elseif result.finalTime < endTime - EPS
    terminationReason = "time_grid_exhausted";
else
...
metadata.benchmarkSnapshots = benchmarkSnapshotManifest;
result.metadata = metadata;
writeJsonFile(fullfile(resultsDir, 'run_metadata.json'), metadata);
...
function gridSummary = summarizeTimeGridForMetadata(timeGrid)
timeGrid = timeGrid(:)';
maxStoredPoints = 5000;
gridSummary = struct( ...
    'numPoints', numel(timeGrid), ...
    'numSteps', max(0, numel(timeGrid) - 1), ...
    'firstTime_s', NaN, ...
    'lastTime_s', NaN, ...
    'storedFullGrid', numel(timeGrid) <= maxStoredPoints, ...
    'time_s', []);
if ~isempty(timeGrid)
    gridSummary.firstTime_s = timeGrid(1);
    gridSummary.lastTime_s = timeGrid(end);
end
if gridSummary.storedFullGrid
    gridSummary.time_s = timeGrid;
end
end
...
function fluxFunction = precipBuildBoundaryFluxFunction(mode, config, fieldName, fallbackConcentration, inletVelocity, epsValue)
fluxFunction = precip_CreateTransportMultiInlet(mode, config, fieldName, ...
    fallbackConcentration, inletVelocity, epsValue);
end

function validateSplitInletRangesOnLeftBoundary(grid, config, epsValue)
precip_ValidateSplitInletMeshAlignment(grid, config, epsValue);
end

function transportProblem = createPhreeqcTransport(grid, stepper, flow, name, initialConcentration, ...
    inletConcentration, diffusionCoefficient, inletVelocity, EPS, upwindMode, inletFluxFunction)
...
if fid ~= -1
    fprintf(fid, ['timestep,time_s,pH_mean,pH_min,pH_max,H_mean_mol_cm3,', ...
        ...
        'Na_mean_mol_cm3,calcite_rate_mean_mol_cm2_s,calcite_signed_rate_mol_s_mean,', ...
        'calcite_net_delta_moles_mean,calcite_precipitated_moles_mean,calcite_dissolved_moles_mean,', ...
...
if isfield(data, 'calcitePrecipitatedMoles')
    precipitatedMolesForLog = data.calcitePrecipitatedMoles;
else
    precipitatedMolesForLog = NaN(size(rateForLog));
end
if isfield(data, 'calciteDeltaMoles')
    netDeltaMolesForLog = data.calciteDeltaMoles;
else
    netDeltaMolesForLog = NaN(size(rateForLog));
end
if isfield(data, 'calciteSignedRate_mol_s')
    signedRateForLog = data.calciteSignedRate_mol_s;
else
    signedRateForLog = NaN(size(rateForLog));
end
fprintf(fid, '%d,%.12g,%.12g,%.12g,%.12g,%.12e,%.12e,%.12e,%.12e,%.12e,%.12e,%.12e,%.12e,%.12e,%.12e,%.12e,%.12g,%.12g\n', ...
    ...
    mean(signedRateForLog, 'omitnan'), mean(netDeltaMolesForLog, 'omitnan'), ...
    mean(precipitatedMolesForLog, 'omitnan'), mean(dissolvedMolesForLog, 'omitnan'), ...
    mean(data.calciteSI, 'omitnan'), mean(data.chargeBalance, 'omitnan'));
fclose(fid);
end

function initializeBenchmarkComparisonLog(csvFile, comparisonTimes)
fid = fopen(csvFile, 'w');
if fid == -1
    warning('RTSPHEM:Precipitate:BenchmarkLogOpenFailed', ...
        'Cannot open benchmark comparison log: %s', csvFile);
    return;
end
...
if isempty(comparisonTimes)
    fprintf(fid, 'NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN\n');
end
fclose(fid);
end

function exported = appendBenchmarkComparisonLogIfDue(csvFile, comparisonTimes, exported, ...
    stepIndex, timeSeconds, porosityValue, permeabilityValue, permeabilityRatio, ...
    rateValue, surfaceAreaValue, grainVolumeValue, grid, levels, areaConfig, initialAreas)
if isempty(comparisonTimes)
    return;
end
due = ~exported & timeSeconds >= comparisonTimes;
if ~any(due)
    return;
end
currentAreas = computeBenchmarkSolidAreaWindows(grid, levels, areaConfig);
firstPoreNet = currentAreas.firstPoreSolidAreaCm2 - initialAreas.firstPoreSolidAreaCm2;
firstThreeNet = currentAreas.firstThreePoresSolidAreaCm2 - initialAreas.firstThreePoresSolidAreaCm2;
fid = fopen(csvFile, 'a');
if fid == -1
    warning('RTSPHEM:Precipitate:BenchmarkLogOpenFailed', ...
        'Cannot append benchmark comparison log: %s', csvFile);
    return;
end
for idx = find(due(:))'
    fprintf(fid, '%.12g,%.12g,%d,%.12g,%.12g,%.12g,%.12g,%.12e,%.12e,%.12e,%.12e,%.12e,%.12e,%.12e\n', ...
        comparisonTimes(idx), comparisonTimes(idx) / 60, stepIndex, timeSeconds, ...
        porosityValue, permeabilityValue, permeabilityRatio, rateValue, ...
        surfaceAreaValue, grainVolumeValue, firstPoreNet, firstThreeNet, ...
        currentAreas.firstPoreSolidAreaCm2, currentAreas.firstThreePoresSolidAreaCm2);
end
fclose(fid);
exported(due) = true;
end

function manifest = buildBenchmarkSnapshotManifest(snapshotDir, comparisonTimes, exported, ...
    finalTimeSeconds, comparisonCsvFile)
comparisonTimes = double(comparisonTimes(:));
exported = logical(exported(:));
numTargets = numel(comparisonTimes);
if numel(exported) ~= numTargets
    exported = false(numTargets, 1);
end

filenames = cell(numTargets, 1);
paths = cell(numTargets, 1);
existsFlag = false(numTargets, 1);
bytes = zeros(numTargets, 1);
capturedStep = NaN(numTargets, 1);
capturedTime = NaN(numTargets, 1);

for iTarget = 1:numTargets
    filenames{iTarget} = precip_BenchmarkSnapshotFilename(comparisonTimes(iTarget));
    paths{iTarget} = fullfile(snapshotDir, filenames{iTarget});
    if isfile(paths{iTarget})
        info = dir(paths{iTarget});
        existsFlag(iTarget) = info.bytes > 0;
        bytes(iTarget) = info.bytes;
    end
end

if exist(comparisonCsvFile, 'file') == 2 && numTargets > 0
    try
        comparisonTable = readtable(comparisonCsvFile);
        if all(ismember({'target_time_s', 'captured_timestep', 'captured_time_s'}, ...
                comparisonTable.Properties.VariableNames))
            for iTarget = 1:numTargets
                rowIndex = find(abs(comparisonTable.target_time_s - comparisonTimes(iTarget)) < 1e-9, 1, 'first');
                if ~isempty(rowIndex)
                    capturedStep(iTarget) = comparisonTable.captured_timestep(rowIndex);
                    capturedTime(iTarget) = comparisonTable.captured_time_s(rowIndex);
                end
            end
        end
    catch ME
        warning('RTSPHEM:Precipitate:SnapshotManifestReadFailed', ...
            'Failed to read benchmark comparison CSV for snapshot manifest: %s', ME.message);
    end
end

due = comparisonTimes <= finalTimeSeconds + 1e-9;
missingDue = due & ~existsFlag;

manifest = struct();
manifest.directory = string(snapshotDir);
manifest.target_time_s = comparisonTimes;
manifest.target_time_min = comparisonTimes ./ 60;
manifest.filename = string(filenames);
manifest.path = string(paths);
manifest.exported_flag = exported;
manifest.file_exists = existsFlag;
manifest.file_bytes = bytes;
manifest.captured_timestep = capturedStep;
manifest.captured_time_s = capturedTime;
manifest.finalTime_s = finalTimeSeconds;
manifest.due = due;
manifest.missing_due = missingDue;
manifest.numTargets = numTargets;
manifest.numDue = sum(due);
manifest.numExportedFiles = sum(existsFlag);
manifest.numMissingDue = sum(missingDue);
end

function areaConfig = configureBenchmarkAreaWindows(config, layoutType)
postDiameter = cfgget(config, 'postDiameter', 0.03);
poreBody = cfgget(config, 'poreBody', 0.018);
poreThroat = cfgget(config, 'poreThroat', 0.004);
inletClearance = cfgget(config, 'zhangInletClearanceCm', max(poreBody, poreThroat));
...
areaConfig = struct();
areaConfig.enabled = any(strcmpi(layoutType, ...
    {'zhang2010_micromodel_local', 'zhang2010_micromodel_full'}));
...
function areas = computeBenchmarkSolidAreaWindows(grid, levels, areaConfig)
areas = struct('firstPoreSolidAreaCm2', NaN, 'firstThreePoresSolidAreaCm2', NaN);
if ~areaConfig.enabled
    return;
end
firstMask = grid.baryT(:, 1) <= areaConfig.firstPoreXMaxCm;
firstThreeMask = grid.baryT(:, 1) <= areaConfig.firstThreePoresXMaxCm;
areas.firstPoreSolidAreaCm2 = computeSolidAreaForTriangles(grid, levels, firstMask);
areas.firstThreePoresSolidAreaCm2 = computeSolidAreaForTriangles(grid, levels, firstThreeMask);
end
...
function diagnostics = computeStabilityDiagnostics(grid, levels, previousConcentration, currentConcentration, ...
    ...
    rateCoefficientTST, thickness, inletConcentrationScale, inletMassFlux, flowConfig, ...
...
diagnostics.flag = joinDiagnosticFlags(diagnostics, inletConcentrationScale);
end
...
function flagText = joinDiagnosticFlags(diagnostics, inletConcentrationScale)
flags = {};
if ~isfinite(diagnostics.cMin) || ~isfinite(diagnostics.cMax)
    flags{end + 1} = 'nonfinite_c'; %#ok<AGROW>
end
concentrationScale = max(abs(inletConcentrationScale), eps);
if diagnostics.cMin < -max(1e-12, 1e-6 * concentrationScale)
    flags{end + 1} = 'negative_c'; %#ok<AGROW>
end
if diagnostics.cMax > max(10 * concentrationScale, concentrationScale + 1e-12)
