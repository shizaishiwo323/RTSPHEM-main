...
flowValidationManifest = getStructField(evidence, ...
    'flowValidationManifest');
...
rows = {
    ...
    'production_stokes', ...
    hasAcceptedProductionStokes(fixedManifest, flowValidationManifest), ...
    'Flow feedback uses a production-validated Stokes backend.'
    'flow_feedback', ...
    hasAcceptedFlowFeedback(fixedManifest, flowValidationManifest), ...
...
tf = getLogicalField(manifest, 'targetGridSpacingSequenceComplete', false) && ...
    ...
    hasGridCaseManifestEvidence(manifest);
end
...
function tf = hasGridCaseManifestEvidence(manifest)
tf = getLogicalField(manifest, 'caseManifestFilesVerified', false) && ...
    hasNonemptyTextListField(manifest, 'caseManifestPaths');
if ~tf
    return;
end
caseManifestPaths = string(manifest.caseManifestPaths(:));
tf = numel(caseManifestPaths) == 3 && ...
    all(contains(caseManifestPaths, "yoon_fixed_geometry_manifest.json"));
end
...
tf = getLogicalField(manifest, 'geometryPackageIsQuantitative', false) && ...
    ...
    hasNonemptyTextField(manifest, 'geometrySourceFigure') && ...
    hasNonemptyTextField(manifest, 'geometrySourceImageFile') && ...
    hasNonemptyTextField(manifest, 'geometryCalibrationCsv') && ...
    hasNonemptyTextField(manifest, 'geometryProcessingScript') && ...
    hasNonemptyTextField(manifest, 'geometryUncertaintyCsv') && ...
...
tf = getLogicalField(manifest, ...
    ...
    getLogicalField(manifest, 'geometryCalibrationTableVerified', false) && ...
    getLogicalField(manifest, 'geometryUncertaintyTableVerified', false) && ...
...
tf = tf && getLogicalField(package, 'assetFilesVerified', false) && ...
    getLogicalField(package, 'calibrationTableVerified', false) && ...
    getLogicalField(package, 'rawExportTableVerified', false) && ...
    getLogicalField(package, 'uncertaintyTableVerified', false) && ...
    hasReferenceRows && numReferenceRows > 0;
note = lower(string(package.note));
tf = tf && ~contains(note, "missing") && ...
    ~contains(note, "incomplete") && ~contains(note, "invalid");
end

function tf = hasAcceptedFlowFeedback(manifest, flowValidationManifest)
tf = getLogicalField(manifest, 'flowFeedbackAccepted', false) && ...
    ...
    hasAcceptedProductionStokes(manifest, flowValidationManifest);
if ~tf
    return;
end
...
function tf = hasAcceptedProductionStokes(manifest, flowValidationManifest)
tf = getLogicalField(manifest, 'finalFlowIsStokes', false) && ...
    ...
    hasAcceptedFlowValidation(flowValidationManifest);
if ~tf
    return;
end
...
[divergenceResidual, hasDivergenceResidual] = getFiniteScalarDouble( ...
    manifest, 'finalFlowMaxDivergenceResidual_s_inv');
[maxAcceptedDivergence, hasDivergenceThreshold] = getFiniteScalarDouble( ...
    manifest, 'maxAcceptedFlowDivergenceResidual_s_inv');
[boundaryClosureError, hasBoundaryClosure] = getFiniteScalarDouble( ...
    manifest, 'finalFlowBoundaryClosureRelativeError');
[maxAcceptedBoundaryClosure, hasBoundaryThreshold] = getFiniteScalarDouble( ...
    manifest, 'maxAcceptedFlowBoundaryClosureRelativeError');
tf = hasLinearResidual && hasResidualThreshold && ...
    hasDivergenceResidual && hasDivergenceThreshold && ...
    hasBoundaryClosure && hasBoundaryThreshold && ...
    maxAcceptedResidual >= 0 && maxAcceptedDivergence >= 0 && ...
    maxAcceptedBoundaryClosure >= 0 && ...
    linearResidual <= maxAcceptedResidual && ...
    divergenceResidual <= maxAcceptedDivergence && ...
    boundaryClosureError <= maxAcceptedBoundaryClosure;
end

function tf = hasAcceptedFlowValidation(manifest)
tf = getLogicalField(manifest, 'productionFlowValidationAccepted', false) && ...
    getLogicalField(manifest, 'requiredCasesComplete', false) && ...
    getLogicalField(manifest, 'isStokes', false) && ...
    ~getLogicalField(manifest, 'isProxy', true) && ...
    getLogicalField(manifest, 'validationSummaryCsvVerified', false) && ...
    hasAcceptedOutputProvenance(manifest);
if ~tf
    return;
end

solverName = lower(string(getFieldOrDefault(manifest, 'solver', "")));
requiredCases = ["empty_channel", "single_obstacle", "blocked_column"];
if ~contains(solverName, "stokes") || ...
        ~contains(solverName, "hyphm") || ...
        ~hasTextListValues(manifest, 'validatedCaseNames', requiredCases)
    tf = false;
    return;
end

[maxLinearResidual, hasLinearResidual] = getFiniteScalarDouble( ...
    manifest, 'maxLinearResidualRelative');
[maxAcceptedLinear, hasLinearThreshold] = getFiniteScalarDouble( ...
    manifest, 'maxAcceptedLinearResidualRelative');
[maxDivergence, hasDivergence] = getFiniteScalarDouble( ...
    manifest, 'maxDivergenceResidual_s_inv');
[maxAcceptedDivergence, hasDivergenceThreshold] = getFiniteScalarDouble( ...
    manifest, 'maxAcceptedDivergenceResidual_s_inv');
[maxBoundaryClosure, hasBoundaryClosure] = getFiniteScalarDouble( ...
    manifest, 'maxBoundaryClosureRelativeError');
[maxAcceptedBoundaryClosure, hasBoundaryThreshold] = getFiniteScalarDouble( ...
    manifest, 'maxAcceptedBoundaryClosureRelativeError');
[minRelativePermeability, hasMinK] = getFiniteScalarDouble( ...
    manifest, 'minRelativePermeability');
[maxRelativePermeability, hasMaxK] = getFiniteScalarDouble( ...
    manifest, 'maxRelativePermeability');

tf = hasLinearResidual && hasLinearThreshold && hasDivergence && ...
    hasDivergenceThreshold && hasBoundaryClosure && hasBoundaryThreshold && ...
    hasMinK && hasMaxK && maxAcceptedLinear >= 0 && ...
    maxAcceptedDivergence >= 0 && maxAcceptedBoundaryClosure >= 0 && ...
    maxLinearResidual <= maxAcceptedLinear && ...
    maxDivergence <= maxAcceptedDivergence && ...
    maxBoundaryClosure <= maxAcceptedBoundaryClosure && ...
    minRelativePermeability >= 0 && maxRelativePermeability <= 1;
end
...
tf = getLogicalField(manifest, 'diffusionFeedbackAccepted', false) && ...
    ...
    getLogicalField(manifest, 'summaryCsvVerified', false) && ...
    hasAcceptedOutputProvenance(manifest);
...
[numSamples, hasNumSamples] = getFiniteScalarDouble(manifest, 'numSamples');
[mixingFractionStep, hasMixingFractionStep] = getFiniteScalarDouble( ...
    manifest, 'mixingFractionStep');
...
tf = hasPhDifference && hasSiDifference && ...
    ...
    hasIphreeqcPeak && hasNumSamples && hasMixingFractionStep && ...
    numSamples == 101 && abs(mixingFractionStep - 0.01) <= 1e-12 && ...
    maxAcceptedPhDifference >= 0 && ...
    ...
    getLogicalField(manifest, 'mixingFractionsCover101', false);
end
...
[maxBoundaryClosureError, hasBoundaryClosureError] = ...
    getFiniteScalarDouble(manifest, ...
    'maxAbsInertBoundaryClosureRelativeError');
[maxAcceptedBoundaryClosureError, hasBoundaryClosureThreshold] = ...
    getFiniteScalarDouble(manifest, ...
    'maxAcceptedBoundaryClosureRelativeError');
...
tf = hasMinConcentration && hasMassError && hasMassThreshold && ...
    hasBoundaryClosureError && hasBoundaryClosureThreshold && ...
    hasMixingError && hasMixingThreshold && hasRejectedCount && ...
    hasRejectedThreshold && minConcentration >= -1e-15 && ...
    maxMassError >= 0 && maxAcceptedMassError >= 0 && ...
    maxBoundaryClosureError <= maxAcceptedBoundaryClosureError && ...
    transportModeIsFiniteVolumeInitial(manifest) && ...
    mixingError <= maxAcceptedMixingError && ...
    rejectedStepCount <= maxAcceptedRejectedCount && ...
    hasRequiredConservativeComponents(manifest);
end

function tf = transportModeIsFiniteVolumeInitial(manifest)
tf = isfield(manifest, 'transportMode') && ...
    strcmp(string(manifest.transportMode), "finite_volume") && ...
    isfield(manifest, 'initialComponentSource') && ...
    strcmp(string(manifest.initialComponentSource), "initial");
end
...
tf = hasQuantitativeTransportAndReactionMode(manifest);
end

function tf = hasQuantitativeTransportAndReactionMode(manifest)
tf = isfield(manifest, 'transportMode') && ...
    strcmp(string(manifest.transportMode), "finite_volume") && ...
    isfield(manifest, 'reactionSubcycling') && ...
    logical(manifest.reactionSubcycling);
end
...
function tf = hasTextListValues(s, fieldName, requiredValues)
tf = false;
if ~isfield(s, fieldName) || isempty(s.(fieldName))
    return;
end
...
values = values(:);
tf = all(ismember(string(requiredValues(:)), values));
end
...
function tf = hasAcceptedOutputProvenance(manifest)
tf = hasGitCommitShaField(manifest, 'sourceCommitSha') && ...
    isfield(manifest, 'outputHashAlgorithm') && ...
    strcmp(string(manifest.outputHashAlgorithm), "SHA-256") && ...
    hasSha256Field(manifest, 'outputEvidenceHashSha256') && ...
    getLogicalField(manifest, 'outputHashInputFilesVerified', false) && ...
    hasNonemptyTextListField(manifest, 'outputHashInputFiles');
if ~tf
    return;
end

if isfield(manifest, 'outputFileSha256') && ...
        ~isempty(manifest.outputFileSha256)
    fileHashes = string(manifest.outputFileSha256(:));
    tf = all(arrayfun(@(value) isSha256Text(value), fileHashes));
else
    tf = false;
end
end

function tf = hasGitCommitShaField(s, fieldName)
tf = isfield(s, fieldName) && isSha1Text(string(s.(fieldName)));
end

function tf = hasSha256Field(s, fieldName)
tf = isfield(s, fieldName) && isSha256Text(string(s.(fieldName)));
end

function tf = isSha1Text(value)
tf = isscalar(value) && ...
    ~isempty(regexp(char(value), '^[0-9a-fA-F]{40}$', 'once'));
end

function tf = isSha256Text(value)
tf = isscalar(value) && ...
