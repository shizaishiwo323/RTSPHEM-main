...
quantitativeRequested = getFieldOrDefault(options, ...
    'isQuantitativeBenchmark', false);
runOptions = configureRunOptions(options, quantitativeRequested);
...
manifest.mineralPhase = spec.mineralPhase;
manifest.solidStateVariable = spec.solidStateVariable;
manifest.legacyLevelSetPathUsed = false;
...
manifest.transportMode = char(string(run.transportMode));
manifest.reactionSubcycling = logical(run.reactionSubcycling);
...
if ~isempty(geometryPackage)
    ...
    manifest.geometrySourceFigure = char(geometryPackage.sourceFigure);
    manifest.geometrySourceImageFile = char(geometryPackage.sourceImageFile);
    manifest.geometryCalibrationCsv = char(geometryPackage.calibrationCsv);
    manifest.geometryProcessingScript = char(geometryPackage.processingScript);
    manifest.geometryUncertaintyCsv = char(geometryPackage.uncertaintyCsv);
    manifest.geometryCalibrationTableVerified = ...
        geometryPackage.calibrationTableVerified;
    manifest.geometryUncertaintyTableVerified = ...
        geometryPackage.uncertaintyTableVerified;
...
manifest.finalFlowMaxDivergenceResidual_s_inv = ...
    run.flowDiagnostics.flowMaxDivergenceResidual_s_inv(end);
manifest.maxAcceptedFlowDivergenceResidual_s_inv = getFieldOrDefault( ...
    options, 'maxAcceptedFlowDivergenceResidual_s_inv', 1e-8);
manifest.finalFlowBoundaryClosureRelativeError = ...
    run.flowDiagnostics.flowBoundaryClosureRelativeError(end);
manifest.maxAcceptedFlowBoundaryClosureRelativeError = getFieldOrDefault( ...
    options, 'maxAcceptedFlowBoundaryClosureRelativeError', 1e-8);
...
manifest.flowBackendProductionValidated = ...
    isProductionStokesFlowValidated(manifest);
...
manifest.case5BoostActive = getFieldOrDefault(run.state, ...
    'case5BoostActive', false);
manifest.case5ActivationTime_s = getFieldOrDefault(run.state, ...
...
manifest.isQuantitativeBenchmark = logical(quantitativeRequested);
if manifest.isQuantitativeBenchmark
    manifest.note = ['Fixed-geometry quantitative-mode Yoon-path output; ' ...
        'readiness still requires the full audit evidence set.'];
else
...
function runOptions = configureRunOptions(options, quantitativeRequested)
runOptions = options;
if ~quantitativeRequested
    return;
end

if isfield(options, 'transportMode') && ...
        ~strcmp(string(options.transportMode), "finite_volume")
    error('RTSPHEM:Precipitate:InvalidQuantitativeBenchmarkOptions', ...
        ['Quantitative benchmark mode requires ', ...
        'transportMode = finite_volume.']);
end
if isfield(options, 'reactionSubcycling') && ...
        ~logical(options.reactionSubcycling)
    error('RTSPHEM:Precipitate:InvalidQuantitativeBenchmarkOptions', ...
        'Quantitative benchmark mode requires reactionSubcycling = true.');
end
runOptions.transportMode = 'finite_volume';
runOptions.reactionSubcycling = true;
end
...
function tf = isProductionStokesFlowValidated(manifest)
solverName = lower(string(manifest.finalFlowSolver));
tf = logical(manifest.finalFlowIsStokes) && ...
    ~logical(manifest.finalFlowIsProxy) && ...
    contains(solverName, "stokes") && ...
    ~contains(solverName, "proxy") && ...
    ~contains(solverName, "darcy") && ...
    isfinite(manifest.finalFlowLinearResidualRelative) && ...
    isfinite(manifest.maxAcceptedFlowLinearResidualRelative) && ...
    manifest.maxAcceptedFlowLinearResidualRelative >= 0 && ...
    manifest.finalFlowLinearResidualRelative <= ...
        manifest.maxAcceptedFlowLinearResidualRelative && ...
    isfinite(manifest.finalFlowMaxDivergenceResidual_s_inv) && ...
    isfinite(manifest.maxAcceptedFlowDivergenceResidual_s_inv) && ...
    manifest.maxAcceptedFlowDivergenceResidual_s_inv >= 0 && ...
    manifest.finalFlowMaxDivergenceResidual_s_inv <= ...
        manifest.maxAcceptedFlowDivergenceResidual_s_inv && ...
    isfinite(manifest.finalFlowBoundaryClosureRelativeError) && ...
    isfinite(manifest.maxAcceptedFlowBoundaryClosureRelativeError) && ...
    manifest.maxAcceptedFlowBoundaryClosureRelativeError >= 0 && ...
    manifest.finalFlowBoundaryClosureRelativeError <= ...
        manifest.maxAcceptedFlowBoundaryClosureRelativeError;
end
