...
function testFreezeGeometryAndMineralInventoryDuringSteadyInitializer(testCase)
cfg = rtm.config.CreateMolinsBenchmarkConfig('partI_strict');
cfg.chemistry.rate_constant_cm_s = 0.1;
cfg.geometry.molarVolume_cm3_mol = 1;
cfg.geometry.maxDisplacementOverH = 2e4;
cfg.time.rt.initialDt_s = 1.0;
cfg.time.rt.maxDt_s = 1.0;
initialState = strictState(10, 10);
initialGeometry = largeSolidInterfaceGeometry();
driver = rtm.driver.ReactiveTransportDriver(cfg, initialState, ...
    initialGeometry, struct());
...
verifyTrue(testCase, info.converged);
verifyEqual(testCase, info.state.mineral_moles, initialState.mineral_moles, ...
    'AbsTol', 0);
verifyEqual(testCase, info.geometry.solid_volume_cm3, ...
    initialGeometry.solid_volume_cm3, 'AbsTol', 0);
verifyEqual(testCase, info.geometry.water_volume_cm3, ...
    initialGeometry.water_volume_cm3, 'AbsTol', 0);
verifyGreaterThan(testCase, info.window_rates_mol_s(1), 0);
verifyEqual(testCase, info.summaries(1).step_results(1).state.mineral_moles, ...
    initialState.mineral_moles, 'AbsTol', 0);
verifyEqual(testCase, ...
    info.summaries(1).step_results(1).geometry.solid_volume_cm3, ...
    initialGeometry.solid_volume_cm3, 'AbsTol', 0);
end
