...
cfg.chemistry = struct('mode', 'external_tst_phreeqc', ...
    ...
    'calciteStoichiometryAbsoluteTolerance_mol', 2e-14, ...
    'calciteStoichiometryRelativeTolerance', 3e-8);
...
cfg.flow = struct('absoluteTolerance_cm3_s', 1e-12, ...
...
verifyEqual(testCase, ...
    manifest.chemistry.calciteStoichiometryAbsoluteTolerance_mol, 2e-14);
verifyEqual(testCase, ...
    manifest.chemistry.calciteStoichiometryRelativeTolerance, 3e-8);
...
verifyEqual(testCase, manifest.phreeqc.engine_type, "iphreeqc_com");
verifyFalse(testCase, manifest.phreeqc.is_available);
verifyEqual(testCase, manifest.phreeqc.database_name, "phreeqc_rates.dat");
...
verifyEqual(testCase, manifest.flow.absoluteTolerance_cm3_s, 1e-12);
verifyEqual(testCase, manifest.flow.relativeTolerance, 1e-8);
verifyEqual(testCase, ...
    manifest.acceptance_tolerances.mass_absolute_tolerance_mol, 1e-14);
verifyEqual(testCase, ...
    manifest.acceptance_tolerances.mass_relative_tolerance, 1e-8);
verifyEqual(testCase, ...
    manifest.acceptance_tolerances.solid_absolute_tolerance_cm3, 1e-14);
verifyEqual(testCase, ...
    manifest.acceptance_tolerances.solid_relative_tolerance, 1e-8);
verifyEqual(testCase, ...
    manifest.acceptance_tolerances.charge_absolute_tolerance_eq, 1e-8);
verifyEqual(testCase, ...
    manifest.acceptance_tolerances.calcite_stoichiometry_absolute_tolerance_mol, ...
    2e-14);
verifyEqual(testCase, ...
    manifest.acceptance_tolerances.calcite_stoichiometry_relative_tolerance, ...
    3e-8);
verifyEqual(testCase, ...
    manifest.acceptance_tolerances.max_displacement_over_h, 0.25);
verifyEqual(testCase, ...
    manifest.acceptance_tolerances.flow_absolute_tolerance_cm3_s, 1e-12);
verifyEqual(testCase, ...
    manifest.acceptance_tolerances.flow_relative_tolerance, 1e-8);
