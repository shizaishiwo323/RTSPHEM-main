...
result.phreeqcPercentError = NaN(numCells, 1);
result.ionicStrength_mol_kgw = NaN(numCells, 1);
result.solutionWaterMass_kg = NaN(numCells, 1);
...
activityFields = {'h', 'ca', 'hco3', 'co3', 'cl', 'na'};
for iField = 1:numel(activityFields)
    name = activityFields{iField};
    result.([name, '_log10_activity']) = NaN(numCells, 1);
    result.([name, '_activity_dimensionless']) = NaN(numCells, 1);
end
result.h_activity_mol_cm3 = NaN(numCells, 1);
result.specificConductance_uS_cm = NaN(numCells, 1);
result.fluidConductivity_S_m = NaN(numCells, 1);
