...
rateCoefficientTST = cfgget(config, 'rateCoefficientTST', 10^(-4)); % legacy TST coefficient used by c_H*1000*k
...
phreeqcCalciteKineticsParameterConvention = char(cfgget(config, ...
    'phreeqcCalciteKineticsParameterConvention', 'phreeqc_rates_cm2_per_mol'));
phreeqcCalciteSurfaceAreaExponent = cfgget(config, 'phreeqcCalciteSurfaceAreaExponent', 1);
phreeqcLegacySpeedScaleFactor = cfgget(config, 'phreeqcLegacySpeedScaleFactor', 20);
phreeqcLegacyCalciteMolarVolumeForSpeed = cfgget(config, ...
...
phreeqcGridMode = normalizePhreeqcGridMode(config);
useSeparatePhreeqcGrid = usePhreeqc && strcmpi(phreeqcGridMode, 'separate') && ...
    ~strcmpi(phreeqcRateLaw, 'tst_match');
...
phreeqcExportSeismoelectric = logical(cfgget(config, 'phreeqcExportSeismoelectric', false));
...
elseif usePhreeqc && strcmpi(phreeqcGridMode, 'level_set')
    phreeqcReactionGrid = gridHyPHM;
    fprintf('PHREEQC reaction grid          : level-set grid (%d triangles)\n', ...
        gridHyPHM.numT);
end
...
seismoelectricChemistryCsvFile = '';
seismoelectricChemistryXlsxFile = '';
...
if usePhreeqc
    ...
    if phreeqcExportSeismoelectric
        seismoelectricChemistryCsvFile = fullfile(phreeqcOutputDir, 'seismoelectric_chemistry.csv');
        seismoelectricChemistryXlsxFile = fullfile(phreeqcOutputDir, 'seismoelectric_chemistry.xlsx');
    end
    phreeqcSession = [];
    ...
    phreeqcOptions = struct( ...
        ...
        'phreeqcCalciteKineticsParameterConvention', phreeqcCalciteKineticsParameterConvention, ...
        'phreeqcCalciteSurfaceAreaExponent', phreeqcCalciteSurfaceAreaExponent, ...
        'molarVolume', molarVolume, ...
        'phreeqcLegacySpeedScaleFactor', phreeqcLegacySpeedScaleFactor, ...
        'phreeqcLegacyCalciteMolarVolumeForSpeed', phreeqcLegacyCalciteMolarVolumeForSpeed, ...
    ...
    if strcmpi(phreeqcRateLaw, 'tst_match')
        ...
        initialPhreeqcGeometryState = computePhreeqcGeometryState( ...
            gridHyPHM, levelSet{1}(:, 1), molarVolume, thickness);
        initialPhreeqcBaseState = collectPhreeqcState(hydrogenTransport, calciumTransport, ...
            carbonTransport, sodiumTransport, chlorideTransport, 0, initialPhreeqcGeometryState);
        initialPhreeqcState = initialPhreeqcBaseState;
        initialPhreeqcFineStateTemplate = initialPhreeqcBaseState;
        initialEquilibriumOptions = phreeqcOptions;
        initialEquilibriumOptions.timeStepIndex = -1;
        initialEquilibriumOptions.timeStepSize = 1;
        if useSeparatePhreeqcGrid
            initialPhreeqcState = AggregatePhreeqcStateToReactionGrid( ...
                initialPhreeqcState, phreeqcReactionProjection);
        end
        initialEquilibriumData = RunPhreeqcCalciteBatch( ...
            initialPhreeqcState, initialEquilibriumOptions);
        if useSeparatePhreeqcGrid
            initialEquilibriumData = ExpandPhreeqcReactionGridResult( ...
                initialEquilibriumData, initialPhreeqcFineStateTemplate, ...
                phreeqcReactionProjection);
        end
        phreeqcSpeciesData = updatePhreeqcStateFromResult( ...
            phreeqcSpeciesData, initialEquilibriumData);
        applyPhreeqcResultToTransports(initialEquilibriumData, hydrogenTransport, ...
            calciumTransport, carbonTransport, sodiumTransport, chlorideTransport, 0);
        phreeqcCalciteRateData = ComputePhreeqcInterfaceRatePerArea( ...
            initialEquilibriumData, initialPhreeqcGeometryState.interface_area_cm2, ...
            initialEquilibriumOptions.timeStepSize, phreeqcOptions, ...
            initialPhreeqcBaseState.h_mol_cm3);
    end
...
if usePhreeqc && exportPhreeqcGridPlot && ~isempty(phreeqcReactionGrid)
    ...
    phreeqcGridPlotHmax = phreeqcGridHmax;
    if ~useSeparatePhreeqcGrid && isfield(gridHyPHM, 'areaT') && ~isempty(gridHyPHM.areaT)
        phreeqcGridPlotHmax = sqrt(max(gridHyPHM.areaT));
    end
    try
        ExportPhreeqcReactionGridPlot( ...
            ...
            phreeqcGridPlotHmax, lengthXAxis, lengthYAxis);
...
metadata.parameters = struct( ...
    ...
    'legacyRateCoefficientTST_configValue', rateCoefficientTST, ...
    'legacyTstRateLaw', "rate_mol_cm2_s = c_H_mol_cm3 * 1000 * coefficient", ...
    'reactionModel', string(reactionModel), ...
    'phreeqcRunGroup', string(phreeqcRunGroup), ...
    'phreeqcRateLaw', string(phreeqcRateLaw), ...
    'legacyPhreeqcTstRateCoefficient_configValue', phreeqcTstRateCoefficient, ...
    ...
    'phreeqcCalciteKineticsParameterConvention', string(phreeqcCalciteKineticsParameterConvention), ...
    'phreeqcCalciteSurfaceAreaExponent', phreeqcCalciteSurfaceAreaExponent, ...
    'phreeqcLegacySpeedScaleFactor', phreeqcLegacySpeedScaleFactor, ...
    'phreeqcLegacyCalciteMolarVolumeForSpeed', phreeqcLegacyCalciteMolarVolumeForSpeed, ...
    'phreeqcInterfaceRateMode', string(phreeqcInterfaceRateMode), ...
    'phreeqcGridMode', string(phreeqcGridMode), ...
    'useSeparatePhreeqcGrid', useSeparatePhreeqcGrid, ...
    ...
    'phreeqcExportSeismoelectric', phreeqcExportSeismoelectric, ...
...
metadata.outputs = struct( ...
    ...
    'seismoelectric_chemistry_csv', string(seismoelectricChemistryCsvFile), ...
    'seismoelectric_chemistry_xlsx', string(seismoelectricChemistryXlsxFile), ...
...
while transportStepper.next
    ...
    if usePhreeqc
        ...
        phreeqcSpeciesData.water_volume_cm3 = phreeqcGeometryState.water_volume_cm3(:);
        phreeqcSpeciesData.interface_area_cm2 = phreeqcGeometryState.interface_area_cm2(:);
        phreeqcSpeciesData.calcite_moles = phreeqcGeometryState.calcite_moles(:);
    ...
    if usePhreeqc && (mod(timeIterationStep, phreeqcExportEvery) == 0 || doExportStep)
        ...
        if phreeqcExportSeismoelectric
            seRtState = struct( ...
                'porosity', por_now, ...
                'permeability_mD', perm_now, ...
                'k_k0', k_k0_now, ...
                'surface_area_cm2', sa_now, ...
                'grain_volume_cm3', vol_now, ...
                'outlet_h_conc_mol_cm3', outletHConc_now, ...
                'hydraulic_tortuosity', tau_now);
            AppendSeismoelectricChemistrySummary(seismoelectricChemistryCsvFile, ...
                timeIterationStep, phreeqcLogTime, seRtState, phreeqcSpeciesData);
        end
    ...
    if ~stopAfterThisStep
        ...
        if shouldApplyNextStep
            ...
            elseif allowAdaptiveTimeGridExtension && remainingTime > EPS
                ...
                for iTransportExtend = 1:numel(transportProblemsToExtend)
...
result.seismoelectricChemistryCsv = seismoelectricChemistryCsvFile;
result.seismoelectricChemistryXlsx = seismoelectricChemistryXlsxFile;

if phreeqcExportSeismoelectric && isfile(seismoelectricChemistryCsvFile)
    try
        writetable(readtable(seismoelectricChemistryCsvFile), seismoelectricChemistryXlsxFile);
    catch ME
        warning('MATLAB:PNMSeismoelectricExcelWrite', ...
            'Failed to write seismoelectric chemistry Excel file: %s', ME.message);
    end
end
...
data.calciteRatePerArea_mol_cm2_s = zeros(numCells, 1);
...
if isfield(data, 'calciteRatePerArea_mol_cm2_s')
    ...
    rateForLog = zeros(size(data.h_mol_cm3));
end
...
repoCandidate = fullfile(projectRoot, 'ReactiveTransport', 'RTM', 'phreeqc', 'database', 'phreeqc_rates.dat');
...
function mode = normalizePhreeqcGridMode(config)
if cfgHasNonEmptyField(config, 'phreeqcGridMode')
    rawMode = lower(strrep(strtrim(char(config.phreeqcGridMode)), '-', '_'));
else
    useSeparate = logical(cfgget(config, 'useSeparatePhreeqcGrid', false));
    if useSeparate
        rawMode = 'separate';
    else
        rawMode = 'level_set';
...
switch rawMode
    case {'separate', 'separate_grid', 'reaction_grid', 'phreeqc_grid'}
        mode = 'separate';
    case {'level_set', 'levelset', 'same_as_level_set', 'transport', ...
            'transport_grid', 'hyphm', 'fine', 'fine_grid', 'same'}
        mode = 'level_set';
    otherwise
        error('RTSPHEM:Phreeqc:UnknownGridMode', ...
            'Unknown PHREEQC grid mode: %s. Use separate or level_set.', rawMode);
end
end
